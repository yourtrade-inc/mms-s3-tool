"""
GET /v1/stock_invoices - 取得所有出倉單 → CSV → S3 upload
Usage: python3 test_stock_invoices.py
"""
import hmac
import hashlib
import base64
...
def fetch_all_stock_invoices():
    """Paginate through all stock invoices (出倉單)."""
    all_items = []
    page = 1
    per_page = 50
    while True:
        path = '/v1/stock_invoices'
        ...
        print(f"  Page {page}: fetched {len(data)} invoices")
...
def build_csv(invoices):
    """Build CSV string from stock invoices. One row per item in each invoice."""
    buf = io.StringIO()
    ...
    for invoice in invoices:
        ...
        processed_at = format_datetime(invoice.get('created_at', ''))
        for item in invoice.get('items', []):
            ...
            writer.writerow([
                ...
                item.get('sku', ''),                   # sku_cd
                invoice.get('pos_shop_id'),                           # stock_cd = pos_shop_id (POS商店ID)
                '',                                    # rank_cd (not in API)
                good_ng,                               # good_ng: 01=良品/02=不良品
                '',                                    # stock_qty (not in API)
                '',                                    # in_qty (N/A for outbound)
                item.get('quantity', 0),                # out_qty
...
def upload_to_s3(csv_content):
    ...
    s3_key = f"stock-files-after/P_ShipmentDaily_{ts}.csv"
...
if __name__ == '__main__':
    print("=== Fetching stock invoices (出倉單) ===")
    invoices = fetch_all_stock_invoices()
